...
namespace Simula.Scripting.Git
{
    ...
    public class SubmoduleCollection : IEnumerable<Submodule>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="Simula.Scripting.Git.SubmoduleCollection"/> class.
        /// </summary>
        ...
        /// <summary>
        /// Gets the <see cref="Simula.Scripting.Git.Submodule"/> with the specified name.
        /// </summary>
        ...
        internal T Lookup<T>(string name, Func<SubmoduleHandle, T> selector, bool throwIfNotFound = false)
        {
            using (var handle = Proxy.git_submodule_lookup(repo.Handle, name))
            {
                ...
                if (throwIfNotFound)
                {
                    throw new GitException("Submodule lookup failed for '{0}'.", name);
                }
