...
using Simula.Scripting.Git.Handlers;
...
namespace Simula.Scripting.Git
{
    ...
    public sealed class Repository : IRepository
    {
        ...
        public static string Init(string workingDirectoryPath, string gitDirectoryPath)
        {
            ...
            // path to the repository. We pass a fully rooted path in order for the Simula.Scripting.Git caller
        ...
        /// <summary>
        /// Stores the content of the <see cref="Repository.Index"/> as a new <see cref="Simula.Scripting.Git.Commit"/> into the repository.
        ...
        /// <returns>The generated <see cref="Simula.Scripting.Git.Commit"/>.</returns>
                ...
                case FastForwardStrategy.Default:
                    if (mergeAnalysis.HasFlag(GitMergeAnalysis.GIT_MERGE_ANALYSIS_FASTFORWARD))
                    {
                        if (annotatedCommits.Length != 1)
                        {
                            ...
                            throw new GitException("Unable to perform Fast-Forward merge with mith multiple merge heads.");
                        }
